...
# Ensure pytest (and PyCharm pytest runner) does not auto-collect this file.
__test__ = False
...
def _resolve_config_path(config_path: str) -> Path:
    """Resolve config path robustly across IDE/CLI cwd differences."""
    cfg_path = Path(config_path)
    if cfg_path.is_absolute():
        return cfg_path.resolve()

    # 1) Keep normal CLI/PyCharm semantics: relative to current working dir.
    cwd_candidate = (Path.cwd() / cfg_path).resolve()
    if cwd_candidate.exists():
        return cwd_candidate

    # 2) Fallback: relative to repository root.
    repo_candidate = (Path(__file__).resolve().parents[1] / cfg_path).resolve()
    if repo_candidate.exists():
        return repo_candidate

    # 3) Return cwd-resolved path for clear FileNotFound errors.
    return cwd_candidate
...
def run_read_checks(config_path: str) -> int:
    _reset_counters()
    ...
    servo_types = list(dyn_cfg.get("servo_types", []))[:num_arm]
    ...
    if (
        ...
        and prev_checkpoint.get("config_path") == str(resolved_cfg)
...
def run_sim_checks(config_path: str) -> int:
    _reset_counters()
...
def main() -> int:
    ...
    resolved_cfg = _resolve_config_path(args.config)
    if args.level in ("read", "sim", "all") and not resolved_cfg.exists():
        print(f"{_RED}Config not found: {resolved_cfg}{_RESET}")
        return 1
    ...
    if args.level in ("read", "all"):
        rc |= run_read_checks(args.config)
    if args.level in ("sim", "all"):
        rc |= run_sim_checks(args.config)
