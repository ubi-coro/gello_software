...
The admittance loop runs on top of Dynamixel position-mode tracking:
  1. Momentum (MOB) observer reads motor current → estimates τ_ext
...
from gello.bilat_4ch.gello_ur5e_observer import LeaderObserver  # noqa: E402
...
def save_thesis_plots(
    *,
    ...
    ax1a.set_title("(a) Raw MOB Observer Output During Trajectory Replay", loc="left")
    ...
    fig1.tight_layout()
    fname1 = ADMITTANCE_PLOTS_DIR / f"mob_observer_replay_failure_{ts_str}.svg"
    ...
    fig2.tight_layout()
    fname2 = ADMITTANCE_PLOTS_DIR / f"mob_observer_tare_offsets_{ts_str}.svg"
    ...
    if n_joints > j and currents.shape[0] == n_samples:
        ...
        fig3.tight_layout()
        fname3 = ADMITTANCE_PLOTS_DIR / f"mob_observer_eta_detail_J3_{ts_str}.svg"
...
def main() -> int:
    ...
    mob: Optional[LeaderObserver] = None
    ...
    try:
        ...
        # ── MOB observer (Momentum-based, needs tau_cmd) ─────
        ...
        motor_eta = np.full(n, 0.65, dtype=float)
        ...
        if args.thesis_plots and plt is not None and len(tau_raw_buf) > 10:
            try:
                ...
                if safe_mob is None:
                    raise RuntimeError("MOB observer not initialized")
                m_kt = kt[:n].copy()
                m_gr = gear_ratio[:n].copy()
                m_eta = motor_eta[:n].copy()
